/*
  ...
  ADF4351 demo by Bryce Cherry

  Commands:
  REF reference_frequency_in_Hz reference_divider (UNDIVIDED/DOUBLE/HALF) - Set reference frequency, reference divider and reference doubler/divide by 2
  ...
  CE (ON/OFF) - enable/disable ADF4351
  CP_CURRENT current_in_mA_floating - adjust charge pump current to suit your loop filter (default library value is 2.5 mA)
...
const word SweepSteps = 14; // SweepSteps * 4 * 6 is the temporary memory calculation (remember to leave enough for BigNumber)
...
void PrintErrorCode(byte value) {
  switch (value) {
    case ADF4351_ERROR_NONE:
      break;
    case ADF4351_ERROR_STEP_FREQUENCY_EXCEEDS_PFD:
      Serial.println(F("Step frequency exceeds PFD frequency"));
      break;
    case ADF4351_ERROR_RF_FREQUENCY:
      Serial.println(F("RF frequency out of range"));
      break;
    case ADF4351_ERROR_POWER_LEVEL:
      Serial.println(F("Power level incorrect"));
      break;
    case ADF4351_ERROR_AUX_POWER_LEVEL:
      Serial.println(F("Auxiliary power level incorrect"));
      break;
    case ADF4351_ERROR_AUX_FREQ_DIVIDER:
      Serial.println(F("Auxiliary frequency divider incorrect"));
      break;
    case ADF4351_ERROR_ZERO_PFD_FREQUENCY:
      Serial.println(F("PFD frequency is zero"));
      break;
    case ADF4351_ERROR_MOD_RANGE:
      Serial.println(F("Mod is out of range"));
      break;
    case ADF4351_ERROR_FRAC_RANGE:
      Serial.println(F("Fraction is out of range"));
      break;
    case ADF4351_ERROR_N_RANGE:
      Serial.println(F("N is of range"));
      break;
    case ADF4351_ERROR_N_RANGE_OVER_3600_MHz:
      Serial.println(F("N is out of range with RF frequency > 3600 MHz"));
      break;
    case ADF4351_ERROR_RF_FREQUENCY_AND_STEP_FREQUENCY_HAS_REMAINDER:
      Serial.println(F("RF frequency and step frequency division has remainder"));
      break;
    case ADF4351_ERROR_PFD_EXCEEDED_WITH_FRACTIONAL_MODE:
      Serial.println(F("PFD exceeds 32 MHz under fractional mode"));
      break;
    case ADF4351_ERROR_PRECISION_FREQUENCY_CALCULATION_TIMEOUT:
      Serial.println(F("Precision frequency calculation timeout"));
      break;
    case ADF4351_WARNING_FREQUENCY_ERROR:
      Serial.println(F("Actual frequency is different than desired"));
      break;
    case ADF4351_ERROR_DOUBLER_EXCEEDED:
      Serial.println(F("Reference frequency with doubler exceeded"));
      break;
    case ADF4351_ERROR_R_RANGE:
      Serial.println(F("R divider is out of range"));
      break;
    case ADF4351_ERROR_REF_FREQUENCY:
      Serial.println(F("Reference frequency is out of range"));
      break;
    case ADF4351_ERROR_REF_MULTIPLIER_TYPE:
      Serial.println(F("Reference multiplier type is incorrect"));
      break;
    case ADF4351_ERROR_PFD_AND_STEP_FREQUENCY_HAS_REMAINDER:
      Serial.println(F("PFD and step frequency division has remainder"));
      break;
    case ADF4351_ERROR_PFD_LIMITS:
...
void setup() {
  ...
  digitalWrite(CEpin, HIGH); // enable the ADF4351
}
  ...
  if (Serial.available() > 0) {
    ...
    if (value != '\n' && ByteCount < CommandSize) {
      ...
      if (strcmp(field, "REF") == 0) {
        ...
        getField(field, 3);
        byte ReferenceHalfDouble = ADF4351_REF_UNDIVIDED;
        if (strcmp(field, "DOUBLE") == 0) {
          ReferenceHalfDouble = ADF4351_REF_DOUBLE;
        }
        else if (strcmp(field, "HALF") == 0) {
          ReferenceHalfDouble = ADF4351_REF_HALF;
        }
        byte ErrorCode = vfo.setrf(ReferenceFreq, ReferenceDivider, ReferenceHalfDouble);
        if (ErrorCode != ADF4351_ERROR_NONE) {
          ValidField = false;
        ...
        if (strcmp(field, "DIVIDED") == 0) {
          AuxFrequencyDivider = ADF4351_AUX_DIVIDED;
        }
        else if (strcmp(field, "FUNDAMENTAL") == 0) {
          AuxFrequencyDivider = ADF4351_AUX_FUNDAMENTAL;
        }
        else {
          ValidField = false;
        }
        ...
        if (ValidField == true) {
          ...
          if (ErrorCode != ADF4351_ERROR_NONE && ErrorCode != ADF4351_WARNING_FREQUENCY_ERROR) {
            ValidField = false;
          }
          PrintErrorCode(ErrorCode);
          if (ValidField == true) {
        ...
        getField(field, 6);
        word PRESCALER_value = atoi(field);
        getField(field, 7);
        if (strcmp(field, "TRUE") == 0 || strcmp(field, "FALSE") == 0) {
          ...
          vfo.setfDirect(R_divider, INT_value, MOD_value, FRAC_value, RF_DIVIDER_value, PRESCALER_value, FRACTIONAL_MODE);
        }
        else {
          ValidField = false;
        }
        ...
        unsigned long OnBurstData[ADF4351_RegsToWrite];
        ...
        unsigned long OffBurstData[ADF4351_RegsToWrite];
        ...
        if (ValidField == true) {
          if (BN_StartFrequency < BN_StopFrequency) {
            char tmpstr[12];
            ultoa(vfo.ADF4351_ChanStep, tmpstr, 10);
            char tmpstr2[12];
            ...
            if (BN_StepSize >= BigNumber(tmpstr)) {
              ...
              free(tempstring2);
              uint32_t regs[(ADF4351_RegsToWrite * SweepSteps)];
              uint32_t reg_temp[ADF4351_RegsToWrite];
              for (word SweepCount = 0; SweepCount < SweepSteps; SweepCount++) {
                ...
                for (int y = 0; y < ADF4351_RegsToWrite; y++) {
                  regs[(y + (ADF4351_RegsToWrite * SweepCount))] = reg_temp[y];
                }
              }
              if (ValidField == true) {
                ...
                while (true) {
                  ...
                  for (word SweepCount = 0; SweepCount < SweepSteps; SweepCount++) {
                    ...
                    for (int y = 0; y < ADF4351_RegsToWrite; y++) {
                      reg_temp[y] = regs[(y + (ADF4351_RegsToWrite * SweepCount))];
                    }
        ...
        if (strcmp(field, "INVERTING") == 0) {
          vfo.setPDpolarity(ADF4351_LOOP_TYPE_INVERTING);
        }
        else if (strcmp(field, "NONINVERTING") == 0) {
          vfo.setPDpolarity(ADF4351_LOOP_TYPE_NONINVERTING);
        }
        else {
          ValidField = false;
        }
